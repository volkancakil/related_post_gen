...
const Hasher = struct {
    const SEEDS: usize = if (@bitSizeOf(usize) == 32) 0x9e3779b9 else 0x517cc1b727220a95;
    hash: usize,

    fn calcHash(key: []const u8) u64 {
    ...
    fn write(self: *Hasher, bytes: []const u8) void {
        ...
        const BYTES_OF_USIZE: usize = if (@bitSizeOf(usize) == 32) 4 else 8;
        var b = bytes;
    ...
    fn writeInt(self: *Hasher, comptime T: type, i: T) void {
        if (@bitSizeOf(usize) == 64 and @bitSizeOf(usize) == 32) {
            self.add(@truncate(i));
